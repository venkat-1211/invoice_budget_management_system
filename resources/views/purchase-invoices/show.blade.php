...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-0">Purchase Invoice {{ $invoice->invoice_number }}</h4>
        <p class="text-muted small mb-0">View purchase invoice details</p>
    </div>
    <div class="d-flex gap-2">
        <x-button href="{{ route('purchase-invoices.index') }}" variant="outline-secondary" icon="arrow-left">
            Back
        </x-button>
        <x-button href="{{ route('payments.create', ['type' => 'purchase', 'invoice_id' => $invoice->id]) }}" variant="success" icon="credit-card">
            Record Payment
        </x-button>
    </div>
</div>
        ...
        <x-card>
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <p class="mb-1 fw-semibold">{{ $invoice->supplier->name }}</p>
                    <p class="mb-1 text-muted small">{{ $invoice->supplier->email }}</p>
                    <p class="mb-1 text-muted small">{{ $invoice->supplier->phone }}</p>
                    <p class="mb-0 text-muted small">{{ $invoice->supplier->address }}</p>
                </div>
