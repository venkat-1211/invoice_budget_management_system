...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted small mb-0">Update purchase invoice {{ $invoice->invoice_number }}</p>
    </div>
...
<x-card>
    <form method="POST" action="{{ route('purchase-invoices.update', $invoice->id) }}" id="invoiceForm">
        @csrf
        @method('PUT')

        <div class="row mb-4">
            <div class="col-md-4">
                <x-select name="supplier_id" label="Supplier" required :options="[$invoice->supplier_id => $invoice->supplier->name]" value="{{ $invoice->supplier_id }}" />
            </div>
            <div class="col-md-4">
