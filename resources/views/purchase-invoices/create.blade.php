...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted small mb-0">Record a new purchase from supplier</p>
    </div>
    <x-button href="{{ route('purchase-invoices.index') }}" variant="outline-secondary" icon="arrow-left">
        Back
    </x-button>
</div>

<x-card>
    <form method="POST" action="{{ route('purchase-invoices.store') }}" id="invoiceForm">
        @csrf

        <div class="row mb-4">
            <div class="col-md-4">
                <x-select name="supplier_id" label="Supplier" required :options="[]" placeholder="Select Supplier" />
            </div>
            <div class="col-md-4">
                <x-input type="date" name="invoice_date" label="Invoice Date" required value="{{ old('invoice_date', now()->toDateString()) }}" />
            </div>
            <div class="col-md-4">
                <x-input type="date" name="due_date" label="Due Date" required value="{{ old('due_date', now()->addDays(30)->toDateString()) }}" />
            </div>
        </div>
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-4">
                <x-input name="reference" label="Reference" value="{{ old('reference') }}" />
            </div>
            <div class="col-md-4">
                <x-input name="exchange_rate" type="number" step="0.000001" label="Exchange Rate" value="1" />
            </div>
        </div>
                ...
                <tbody id="itemsBody">
                    <tr class="item-row">
                        <td>
                            ...
                            <input type="text" name="items[0][description]" class="form-control form-control-sm item-desc" required placeholder="Description">
                        </td>
                        <td><input type="number" name="items[0][quantity]" class="form-control form-control-sm item-qty" value="1" min="0.01" step="0.01" required></td>
                        <td><input type="text" name="items[0][unit]" class="form-control form-control-sm item-unit" value="pcs"></td>
                        <td><input type="number" name="items[0][unit_price]" class="form-control form-control-sm item-price" value="0" min="0" step="0.01" required></td>
                        <td><input type="number" name="items[0][tax_rate]" class="form-control form-control-sm item-tax" value="0" min="0" max="100" step="0.01"></td>
                        <td><input type="number" name="items[0][discount_percent]" class="form-control form-control-sm item-disc" value="0" min="0" max="100" step="0.01"></td>
            ...
            <div class="col-md-4">
                <table class="table table-borderless">
                    <tr>
                        ...
                        <td class="text-end" style="width: 150px;"><span id="subtotal">0.00</span></td>
                    </tr>
                    <tr>
                        <td class="text-end fw-semibold">Tax:</td>
                        <td class="text-end"><span id="taxTotal">0.00</span></td>
                    </tr>
                    <tr>
                        <td class="text-end fw-semibold">Discount:</td>
                        <td class="text-end"><span id="discountTotal">0.00</span></td>
                    </tr>
                    <tr class="border-top">
                        <td class="text-end fw-bold h5">Total:</td>
                        <td class="text-end fw-bold h5"><span id="grandTotal">0.00</span></td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-12">
                <x-textarea name="notes" label="Notes" rows="3" value="{{ old('notes') }}" />
            </div>
        </div>

        <div class="d-flex justify-content-end gap-2">
            <x-button type="button" variant="outline-secondary" onclick="window.history.back()">Cancel</x-button>
            <x-button type="submit" variant="primary" icon="check-lg">Create Invoice</x-button>
        </div>
    </form>
</x-card>
@endsection
